...
from operational_resilience_mapping.adapters.local.audit import (
    LocalAuditAdapter,
)
...
_ANCHOR_ENV = "RESILIENCE_AUDIT_ANCHOR"
...
def _record_three(settings: Settings) -> LocalAuditAdapter:
    ...
    for regulator in (Regulator.APRA_CPS230, Regulator.DORA):
        studio.propose_tolerances(
            sample_cases.SERVICE, resilience_map, regulator, actor=sample_cases.ACTOR
        )
    return audit
    ...
    with pytest.raises(AuditChainError):
        audit.record(
            AuditEvent(
                ...
                redacted_summary="post-truncation append",
            )
        )
