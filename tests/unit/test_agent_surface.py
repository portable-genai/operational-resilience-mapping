...
from operational_resilience_mapping.agent import (
    SKILLS,
    TOOL_FUNCTIONS,
    agent_card_document,
    assess_resilience,
...
def test_the_card_document_is_json_safe_and_names_the_region() -> None:
    ...
    assert document["name"] == "operational-resilience-mapping"
...
def test_the_agent_path_routes_an_escalation_rather_than_only_flagging_it() -> None:
    result = assess_resilience(
        sample_cases.SERVICE.id,
        sample_cases.SERVICE.name,
...
def test_the_agent_path_derives_tolerances_from_the_engine() -> None:
    result = assess_resilience(
        sample_cases.SERVICE.id,
        sample_cases.SERVICE.name,
        settings=local_settings(),
    )
    assert result["tolerances"], "the agent surface returned no engine-derived tolerances"


def test_an_unknown_regulator_is_rejected() -> None:
