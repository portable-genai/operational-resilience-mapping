...
from operational_resilience_mapping.adapters.gcp.identity import (
    _AUDIENCE_ENV,
...
from operational_resilience_mapping.ports.identity import (
    VERIFIED,
...
_API_MODULE = "operational_resilience_mapping.api.app"
...
_REBOUND_SETTINGS = "\n".join(
    [
        ...
        *[
        ...
        # The data ports are rebound to the SDK-free local adapters across profiles too, so this
        # identity-focused fixture never needs a cloud SDK; only the identity port stays managed.
        "  map_store:",
        *[f"    {p}: {_PKG}.adapters.local.map_store:LocalMapStore" for p in ("local", "gcp")],
        f"    onprem: {_PKG}.adapters.onprem.map_store:OnPremMapStore",
        "  extraction:",
        *[
        ...
        "  asset_inventory:",
        *[
            f"    {p}: {_PKG}.adapters.local.asset_inventory:LocalAssetInventory"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.asset_inventory:OnPremAssetInventory",
        "  register:",
        *[
            f"    {p}: {_PKG}.adapters.local.register:LocalRegisterAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.register:OnPremRegisterAdapter",
        "  compliance:",
        *[
            f"    {p}: {_PKG}.adapters.local.compliance:LocalComplianceAdapter"
            for p in ("local", "gcp")
        ],
        f"    onprem: {_PKG}.adapters.onprem.compliance:OnPremComplianceAdapter",
        "  generation:",
        *[
...
def test_a_verified_assertion_is_served_so_the_refusals_above_mean_something(
    ...
    A verified assertion is SERVED, and the actor is the VERIFIED subject from the assertion,
    never anything a client wrote. (Rule R8 routing on the serving path is proven separately in
    ``test_review_routing.py`` on the offline profile, where the consequential pipeline runs.)
    """
    ...
    body = resp.json()
    assert body["actor"], "a verified assertion must resolve to a principal actor"
    assert "@" in body["actor"], "the actor is the verified subject's email from the assertion"
