...
from operational_resilience_mapping.adapters._review_payload import (
    result_to_review,
)
...
def test_the_whole_pipeline_answers_on_local_and_fails_fast_on_onprem() -> None:
    local = build_studio(build_container(local_settings()))
    resilience_map, _reconciliation, _gaps = local.build_map(
        sample_cases.SERVICE, sample_cases.SCOPE, actor=sample_cases.ACTOR
    )
    proposal, review_ref = local.propose_tolerances(
        sample_cases.SERVICE, resilience_map, Regulator.APRA_CPS230, actor=sample_cases.ACTOR
    )
    assert proposal.requires_human_review is True
    assert proposal.tolerances, "an offline answer must still derive tolerances"
    assert proposal.citations, "an offline answer must still be cited"
    assert review_ref, "the consequential proposal must be routed (rule R8)"

    onprem = build_studio(build_container(local_settings(profile="onprem")))
    with pytest.raises(NotImplementedError):
        onprem.build_map(sample_cases.SERVICE, sample_cases.SCOPE, actor=sample_cases.ACTOR)
