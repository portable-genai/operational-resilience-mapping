...
``RESILIENCE_PROFILE`` is NO CHOICE rather than a silent ``local``. Every
...
``RESILIENCE_S2S_TOKEN`` is wrong in the most dangerous direction:
...
from .schemas import HealthResponse, ToleranceRequest, ToleranceResponse
...
_INSECURE_DEMO_ENV = "RESILIENCE_ALLOW_INSECURE_DEMO"
_CORS_ORIGINS_ENV = "RESILIENCE_CORS_ORIGINS"
_ALLOWED_CALLERS_ENV = "RESILIENCE_S2S_ALLOWED_CALLERS"
_AUDIENCE_ENV = "RESILIENCE_S2S_AUDIENCE"
_API_HOST_ENV = "RESILIENCE_API_HOST"
...
def require_service_caller(request: Request) -> None:
    ...
    stays OPEN when ``RESILIENCE_S2S_TOKEN`` is unset (loopback dev with zero
    secrets), so an UNSET ``RESILIENCE_PROFILE`` must never be allowed to
    ...
    ``RESILIENCE_ALLOW_INSECURE_DEMO=1``.
    """
    if not _request_choice(request).service_auth_configured:
        raise HTTPException(
            ...
            detail=(
                ...
                "RESILIENCE_PROFILE is not set, so no authentication scheme "
                "has been chosen"
            ),
        )
...
_SERVICE_NAME = "operational-resilience-mapping"
...
_APP_TITLE = "Operational Resilience Studio"
...
# pill names is what answered, never what configuration says would. Offline, the generation stub
# notes `deterministic-offline-stub` on `/v1/tolerance`; a request that noted nothing sends
# neither header, and the pill keeps showing the configured `generator_model` from `/healthz`.
...
#
# Note what is NOT in this expression: RESILIENCE_S2S_TOKEN. A service
...
@app.post("/v1/tolerance", response_model=ToleranceResponse, tags=["artifacts"])
def propose_tolerance(
    ...
    proposal, review_ref = studio.propose_tolerances(
        service,
        resilience_map,
        _parse_regulator(request.regulator),
        actor=principal.actor,
        tenant=principal.tenant,
    )
...
def _parse_regulator(raw: str) -> Regulator:
    try:
        return Regulator(raw)
    except ValueError as exc:
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            detail=f"unknown regulator {raw!r}; use one of {[r.value for r in Regulator]}",
        ) from exc


@app.get("/v1/whoami", tags=["ops"])
def whoami(principal: Annotated[Principal, Depends(get_principal)]) -> dict[str, str]:
    """Echo the VERIFIED end-user principal, so identity resolution can be exercised on its own.

    A read-only surface that depends on nothing but ``get_principal``: it is the endpoint the
    IAP-identity suite drives, because the consequential pipeline needs data ports that the
    managed profile only binds at deploy time. The client-asserted actor is never read.
    """
    return {"actor": principal.actor, "tenant": principal.tenant}
