...
class AgentCard:
    ...
    provider: str = "operational-resilience-mapping"
    skills: tuple[AgentSkill, ...] = field(default_factory=tuple)


SKILLS: tuple[AgentSkill, ...] = (
    AgentSkill(
        id="assess_resilience",
        name="Resilience assessment",
        description=(
            "Build the resilience map for an important business service, derive its impact "
            "tolerances from the deterministic per-regulator engine, and ROUTE the consequential "
            "proposal to human review (rule R8). Every tolerance value is computed by pure "
            "stdlib code, never by a model."
        ),
    ),
    AgentSkill(
...
def build_agent_card(settings: Settings | None = None) -> AgentCard:
    ...
    return AgentCard(
        name="operational-resilience-mapping",
...
def _resolve_url(settings: Settings) -> str:
    ...
    return f"https://operational-resilience-mapping.{settings.region}.internal.example/a2a"
