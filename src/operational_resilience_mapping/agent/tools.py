...
DEFAULT_ACTOR = "operational-resilience-mapping-agent"
...
def assess_resilience(
    service_id: str,
    service_name: str,
    regulator: str = "APRA_CPS230",
    scope: str = "projects/fictional",
    actor: str = DEFAULT_ACTOR,
    tenant: str = "",
    settings: Settings | None = None,
) -> dict[str, Any]:
    """Build the resilience map and propose impact tolerances, routing the proposal to review.

    Ingests the technology inventory, third-party register and process documents, reconciles the
    dependency chain deterministically, derives the impact tolerances for the named regulator
    from the pure engine, and ROUTES the consequential proposal to the human-review console
    (rule R8). The model narrates only; every tolerance value is the engine's.

    Args:
      service_id: The important business service identifier.
      service_name: The human label for the service.
      regulator: One of APRA_CPS230, DORA, UK_OPRES.
      scope: The asset / register scope to ingest.
    ...
    proposal, review_ref = studio.propose_tolerances(
        service, resilience_map, _parse_regulator(regulator), actor=actor, tenant=tenant
    )
    payload = _redacted(to_jsonable(proposal))
...
def _parse_regulator(raw: str) -> Regulator:
    try:
        ...
        raise ValueError(
            f"unknown regulator {raw!r}; use one of {[r.value for r in Regulator]}"
        ) from exc
...
TOOL_FUNCTIONS = (assess_resilience, verify_audit_trail)
