...
One env var (``RESILIENCE_PROFILE``) selects the adapter family for every
...
#: The name the offline generation stub answers under. ``generator_model`` reports it under
#: ``local`` and the stub notes it as the model that answered (``X-Answered-By``), so it is
#: spelled once here and the two cannot drift.
...
class ProfileChoice:
    ...
    The variable is ``RESILIENCE_PROFILE``, named once in
    :data:`_PROFILE_ENV`.
    ...
    ``os.environ.get("RESILIENCE_PROFILE", "local")``: that fallback reads an
...
_PKG = "operational_resilience_mapping"
...
DEFAULT_BINDINGS: dict[str, dict[str, str]] = {
    ...
    "map_store": {
        "local": f"{_PKG}.adapters.local.map_store:LocalMapStore",
        "gcp": f"{_PKG}.adapters.gcp.map_store:CloudMapStore",
        "onprem": f"{_PKG}.adapters.onprem.map_store:OnPremMapStore",
    },
    "extraction": {
    ...
    "asset_inventory": {
        "local": f"{_PKG}.adapters.local.asset_inventory:LocalAssetInventory",
        "gcp": f"{_PKG}.adapters.gcp.asset_inventory:CloudAssetInventory",
        "onprem": f"{_PKG}.adapters.onprem.asset_inventory:OnPremAssetInventory",
    },
    "register": {
        "local": f"{_PKG}.adapters.local.register:LocalRegisterAdapter",
        "gcp": f"{_PKG}.adapters.gcp.register:CloudRegisterAdapter",
        "onprem": f"{_PKG}.adapters.onprem.register:OnPremRegisterAdapter",
    },
    "compliance": {
...
def _read_settings_file(path: Path | None = None) -> dict[str, Any]:
    ...
    An EXPLICIT path (argument or ``RESILIENCE_SETTINGS``) that does not
...
class Settings:
    ...
    def load(cls, path: Path | None = None) -> Settings:
        ...
        settings = cls(
            ...
            register_url=str(data.get("register_url") or ""),
            compliance_url=str(data.get("compliance_url") or ""),
...
class Container:
    ...
    @cached_property
    def map_store(self) -> MapStorePort:
        adapter = self._bind("map_store")
        assert isinstance(adapter, MapStorePort)
        return adapter
    ...
    @cached_property
    def asset_inventory(self) -> AssetInventoryPort:
        adapter = self._bind("asset_inventory")
        assert isinstance(adapter, AssetInventoryPort)
        return adapter

    @cached_property
    def register(self) -> RegisterReadPort:
        adapter = self._bind("register")
        assert isinstance(adapter, RegisterReadPort)
        return adapter
