...
(``RESILIENCE_REVIEW_ROUTING=off``), the container binds :class:`DisabledReviewRouter` instead of
...
than failing an already-derived, already-audited tolerance proposal, but it is never invisible: the
caller reports ``failed`` and an empty reference, which nobody can mistake for a reviewed result.
"""
