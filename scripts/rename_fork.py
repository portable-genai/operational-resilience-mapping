...
The catalog id (``Rgc9``) is left alone unless ``--catalog-id`` is given: it names the entry in
...
_OLD_PACKAGE = "operational_resilience_mapping"
_OLD_ENV_PREFIX = "RESILIENCE_"
_OLD_BARE_PREFIX = "RESILIENCE"
_OLD_RESOURCE = "rgc9-svc"
_OLD_DIST = "operational-resilience-mapping"
_OLD_CATALOG_ID = "Rgc9"
...
def _rewrite_text(text: str, args: argparse.Namespace) -> tuple[str, int]:
    ...
    # The env prefix, only where it PREFIXES an upper token (RESILIENCE_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource",
        required=True,
        help="new cloud resource stem (Terraform name_prefix), e.g. acme-resilience",
    )
