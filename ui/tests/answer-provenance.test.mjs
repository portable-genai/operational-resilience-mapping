...
test("only this console's own API path on its own origin counts", () => {
  assert.equal(isConsoleApi("/api/agent/v1/tolerance", HERE), true);
  ...
  assert.equal(isConsoleApi("http://elsewhere.test/api/agent/v1/tolerance", HERE), false);
...
test("the wrapper reports an answer from the console's API and ignores other origins", async () => {
  ...
  await window.fetch("/api/agent/v1/tolerance", { method: "POST" });
  await window.fetch("http://elsewhere.test/api/agent/v1/tolerance");
  stop();
...
test("the wrapper is installed once however many listeners, and restored by the last", async () => {
  ...
  await window.fetch("/api/agent/v1/tolerance");
  stopFirst();
