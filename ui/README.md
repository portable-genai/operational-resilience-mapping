...
| `app/page.tsx` | The console itself. It reads the service's agent card for its own title, so no product name is hardcoded here, and posts `POST /v1/tolerance` with the service id and name, the asset and register scope, a regulator from the accepted list and optional document ids, prefilled with the fictional Retail Payments service the local profile maps. |
