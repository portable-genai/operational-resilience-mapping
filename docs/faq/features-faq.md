...
1. **Ingestion and reconciliation** (`domain/ingestion_service.py`): three sources become typed
   ...
   from `third-party-risk-ddq`'s register over `RegisterReadPort`, and the process, people and facilities nodes from
...
5. **Concentration and exit** (`domain/concentration_exit/`): four deterministic dimensions,
   ...
   outsourcing expectation it offends and grounded afterwards on `compliance-advisory`'s cited answer.
...
- **The regulatory basis is cited, never invented.** The grounding text comes from `compliance-advisory` over
...
- **It will not invent regulatory text.** A concentration finding or a tolerance basis cites the
  answer `compliance-advisory` returned, and an unavailable source escalates.
- **It will not auto-execute a consequential result.** A tolerance proposal is ALWAYS reviewed
  ...
  CRITICAL; both are ROUTED to the `human-review-console` in the same call that produced them (rule R8).
...
|---|---|---|
| The resilience map for an important business service | **this repo (`operational-resilience-mapping`)** | it IS the system of record. Consumers read the map from here rather than rebuilding one. |
| Impact tolerances and stay-within-tolerance scenario testing | **this repo (`operational-resilience-mapping`)** | derived by pure engines from the map plus a regulator pack. |
| Concentration and exit findings | **this repo (`operational-resilience-mapping`)** | the `domain/concentration_exit/` module, kept here so a concentration finding can aggravate a scenario instead of living in a second tool. |
| The third-party / outsourcing register | `third-party-risk-ddq` third-party risk and DDQ | read as data over `RegisterReadPort` (`RGC8_REGISTER_URL`). `third-party-risk-ddq` is unbuilt, so the offline fixture register is the frozen contract. |
| The regulatory corpus and grounded rule text | `compliance-advisory` | read over `CompliancePort` (`RSK1_COMPLIANCE_URL`). This repo cites what `compliance-advisory` returns; it does not track the corpus. |
...
| Prompt-injection defence and output filtering | `agent-guardrail-gateway` agent guardrail gateway | **not wired today.** It becomes mandatory the moment untrusted free text reaches the narrator (rule R1), and the managed design does pass extracted document text to it. |
| Grounded retrieval over an enterprise corpus | `enterprise-knowledge-base` | not wired; this service reasons over its own graph and cites `compliance-advisory` for rule text. |
...
persistence), routes for the scenario and concentration engines, the `agent-guardrail-gateway` binding, and
